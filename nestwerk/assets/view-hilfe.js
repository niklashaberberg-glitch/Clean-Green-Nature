/* =====================================================================
   Nestwerk – Hilfe: Fenster, Verlauf und Weitergabe an den Service
...
(function (NW) {
  ...
  const U = NW.util, ui = NW.ui, H = NW.hilfe, R = NW.recht;
  ...
  function vorgangsnummer() {
    ...
    return 'NW-' + U.isoDate(d).replace(/-/g, '') + '-' + teil;
  }
  ...
  function beginnen() {
    ...
    verlauf = [{
      von: 'bot', art: 'gruss',
      text: 'Hallo. Ich beantworte die häufigsten Fragen zu Nestwerk – zu Tarifen, zum Bewerben, zum '
  ...
  function nachrichtText(angaben) {
    const k = kern();
    const z = [];
    z.push('Support-Anfrage über die Hilfe von Nestwerk');
    z.push('Vorgang: ' + vorgang);
    z.push('Datum: ' + U.dateDE(U.isoDate(NW.now())));
    ...
    z.push('');
    z.push('---');
    z.push('Zusammengestellt von der Hilfe in Nestwerk. Es wurden keine Profildaten,');
      ...
      html: h`<div class="seite seite--schmal">
        ...
        <div class="hinweisbox">${ico('info')}
          ...
          <p>Die Hilfe rechnet im Browser und ordnet deine Frage einer von
            ${NW.hilfe.THEMEN.length} hinterlegten Antworten zu. Sie erfindet nichts, und wo sie unsicher ist,
  ...
  A_('hilfe-senden', () => {
    ...
    const a = R.angaben();
    const ziel = a.service || 'info@nestwerk.de';
  ...
  A_('hilfe-sichern', () => {
    ui.dateiSichern('nestwerk-anfrage-' + (vorgang || 'entwurf') + '.txt',
  ...
  NW.viewHilfe = { zeichnen, frageStellen, nachrichtText, kern, oeffnen: () => { offen = true; zeichnen(); } };
})(window.NW = window.NW || {});
