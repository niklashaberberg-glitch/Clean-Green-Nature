...
(function (NW) {
  ...
  const U = NW.util, ui = NW.ui, S = NW.store, W = NW.werkzeuge, P = NW.plan, G = NW.geo;
        ...
        <div class="marktleiste">
          <label class="feld feld--flach"><span>Stadt</span>
            <select data-tu-change="markt-stadt">
              ${NW.data.staedte.map((c) => h`<option value="${c}" ${sicht.stadt === c ? 'selected' : ''}>${c}</option>`)}
            </select></label>
  ...
  NW.viewMarkt = { sicht, stadtWaehlen };
})(window.NW = window.NW || {});
