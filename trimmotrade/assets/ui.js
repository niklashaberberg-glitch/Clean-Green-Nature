/* =====================================================================
   TrimmoTrade – Oberfläche, Kern
   Schale, Router, geteilte Bausteine. Ansichten registrieren sich unter
   TT.ui.ansichten und liefern fertiges Markup; alles Interaktive läuft
...
(function (TT) {
  ...
  function kartenMarken(l, b) {
    ...
    const st = TT.konto ? (l.anbieter.stufe === undefined ? (l.anbieter.verifiziert ? 3 : 1) : l.anbieter.stufe) : null;
  ...
  function inseratsKarte(l, b, optionen) {
    ...
    const hervor = TT.plan.istHervorgehoben(l);
  ...
  function sperrHinweis(leistungId, text) {
    const l = TT.plan.leistung(leistungId) || {};
  ...
  function alsText(daten, dateiname) {
    dialog({
      ...
      inhalt: h`<p>In dieser Umgebung darf die Seite keine Datei ablegen. Der vollständige Inhalt steht
        hier zum Kopieren – speichere ihn als <code>${dateiname || 'trimmotrade-daten.json'}</code>.</p>
  ...
  function zeichnen(erzwingen) {
    ...
    ui.aktuell = name;
    ui.params = r;
    document.body.classList.toggle('ist-angemeldet', !(TT.konto && !TT.konto.angemeldet()));
  ...
  /* Nur dort, wo die Dateien auch liegen: im Stamm der Website, neben
     index.html. In der Einzeldatei, im Artefakt und in einer
     eingebetteten Kopie wären es tote Verweise – und ein toter Verweis
  ...
  function schaleZeichnen() {
    ...
    aktualisiereZaehler();
    bandZeichnen();
    if (TT.viewHilfe) TT.viewHilfe.zeichnen();
  }
  ...
  function aktualisiereZaehler() {
    ...
    if (tarifknopf) {
      const plus = TT.plan.istPlus();
      tarifknopf.classList.toggle('is-plus', plus);
      tarifknopf.title = U.t(plus ? 'TrimmoTrade Plus aktiv' : 'Tarife ansehen');
    ...
    let neu = 0;
    s.agenten.forEach((a) => { if (a.aktiv) neu += TT.store.agentTreffer(a).neu.length; });
    setze('agenten', neu);
  }
  ...
  function themeSetzen(wert) {
    ...
    TT.store.set({ theme: wert }, 'theme');
  }

  function spracheWechseln(ziel) {
    if (TT.i18n.sprache() === ziel) return;
    TT.i18n.setze(ziel);
    ...
    TT.data.neuAufbauen();
    if (TT.hilfe) TT.hilfe.indizieren();
  ...
  function themeWechseln() {
    const jetzt = TT.store.get().theme;
  ...
  function paletteOeffnen(vorgabe) {
    ...
    function vorschlaege(q) {
      ...
      if (n.length >= 2) {
        ...
        const objekte = TT.data.listings.filter((l) => TT.analyse.passtText(l, q)).slice(0, 6);
  ...
  const AKTIONEN = {
    ...
    sperre(el) {
      ...
      const l = TT.plan.leistung(id) || { name: 'Diese Funktion' };
    ...
    'plus-beenden'() {
      TT.plan.wechseln('frei');
      if (TT.plan.plusQuelle() === 'gruender') {
    ...
    'band-zu'(el) {
      TT.store.update((s) => { s.hinweiseGelesen[el.dataset.was] = U.isoDate(TT.now()); }, 'hinweis');
      bandZeichnen();
    },
    ...
    'warum-top'() {
      dialog({
        ...
        inhalt: h`<p>Diese Inserate stehen dort, weil die anbietende Seite für den Platz bezahlt hat. TrimmoTrade
          ...
          <ul class="liste-schlicht">
            ...
            <li>Ein bezahlter Platz sagt nichts über die Wohnung. Prüfhinweis, Vergleichsmiete und Chancen
              rechnet TrimmoTrade hier genauso wie überall.</li>
            <li>Kein Inserat verschwindet, weil ein anderes bezahlt hat. Die Liste bleibt vollständig.</li>
          </ul>
          <p class="fein">Höchstens ${TT.plan.TOP_MAX} bezahlte Plätze über einer Trefferliste. Mehr, und der
    ...
    'daten-export'() {
      dateiSichern('trimmotrade-daten.json', JSON.stringify(TT.store.get(), null, 2), 'application/json');
    },
  ...
  function start() {
    ...
    let wurzel = document.getElementById('trimmotrade');
    if (!wurzel) {
      wurzel = document.createElement('div');
      wurzel.id = 'trimmotrade';
