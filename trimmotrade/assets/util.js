...
(function (TT) {
  ...
  function nf(art) {
    const ort = TT.i18n && TT.i18n.sprache() === 'en' ? 'en-GB' : 'de-DE';
  ...
  function dateDE(iso) {
    ...
    return d.toLocaleDateString(TT.i18n && TT.i18n.sprache() === 'en' ? 'en-GB' : 'de-DE',
  ...
  function monthDE(iso) {
    const d = new Date(iso + (iso.length === 10 ? 'T12:00:00' : ''));
    if (isNaN(d)) return String(iso);
    return d.toLocaleDateString(TT.i18n && TT.i18n.sprache() === 'en' ? 'en-GB' : 'de-DE', { month: 'long', year: 'numeric' });
  }
  ...
  function uebersetze(text) {
    return TT.i18n ? TT.i18n.t(text) : text;
  }
