/* =====================================================================
   TrimmoTrade – Ansichten: Profil, Inserieren, Rechner
...
(function (TT) {
      ...
      html: h`<div class="seite seite--schmal">
        ...
        <section class="block">
          ...
          <fieldset class="filter__gruppe"><legend>Städte</legend>
            <div class="chips">
              ${TT.data.staedte.map((c) => h`<button type="button" class="chip ${p.staedte.indexOf(c) >= 0 ? 'is-an' : ''}"
          ...
          <fieldset class="filter__gruppe"><legend>Muss vorhanden sein</legend>
            <div class="chips">
              ${TT.data.AUSSTATTUNG.slice(0, 22).map((a) => h`<button type="button" class="chip ${p.mussHaben.indexOf(a) >= 0 ? 'is-an' : ''}"
          ...
          <fieldset class="filter__gruppe"><legend>Wäre schön</legend>
            <div class="chips">
              ${TT.data.AUSSTATTUNG.slice(0, 22).map((a) => h`<button type="button" class="chip ${p.schoenWaere.indexOf(a) >= 0 ? 'is-an' : ''}"
        ...
        <section class="block">
          ...
          <p class="block__unter">Statt Luftlinie rechnet TrimmoTrade die Fahrzeit zu den Orten, an denen du regelmäßig
          ...
          <div class="formraster formraster--drei">
            ...
            <label class="feld"><span>Ort</span>
              <select id="anker-ort">
                ${TT.geo.DISTRICTS.map((d) => h`<option value="${d.key}">${d.city} – ${d.name}</option>`)}
              </select></label>
        ...
        const k = TT.konto.aktuell();
        const st = TT.konto.stufe(k.stufe);
        return h`<section class="block">
          <div class="block__kopfzeile">
            <h2>${ico('person')}Konto</h2>
            ${ui.badge(st.name, st.ton)}
          </div>
          <p class="block__unter">${U.t('Angemeldet als')} <b>${TT.konto.anzeigeName()}</b>${k.mail ? ' · ' + k.mail : ''}.
        ...
        <section class="block">
          ...
          <p class="block__unter">Zwei bis drei Sätze über dich. TrimmoTrade baut sie in jedes Anschreiben ein.</p>
      ...
      html: h`<div class="seite seite--schmal">
        ...
        h`<option>${x}</option>`)}
          ...
          <fieldset class="filter__gruppe" data-gruppe="wohnung"><legend>Ausstattung</legend>
            <div class="chips">
              ${TT.data.AUSSTATTUNG.slice(0, 22).map((a) =>
  ...
  A_('gewichtung-zurueck', () => {
    profilSpeichern((p) => { p.gewichtung = Object.assign({}, TT.data.profilVorlage.gewichtung); });
    ui.neuZeichnen();
  ...
  A_('anker-neu', () => {
    ...
    const d = TT.geo.districtByKey[key];
  ...
  A_('hervorheben', (el) => {
    ...
    ui.dialog({
      ...
      inhalt: h`<p>Wer schneller vermieten oder verkaufen will, kann für Sichtbarkeit zahlen. Was du dabei
        ...
        <p class="fein">${TT.recht ? TT.recht.preisHinweis() : ''}</p>
        <div class="hinweisbox">${ico('info')}
          ...
          <p>Bezahlte Platzierung in Suchergebnissen muss als solche erkennbar sein (§ 5b Abs. 1 Nr. 6 und
            Abs. 2 UWG). TrimmoTrade löst das nicht mit einem kleinen Sternchen, sondern mit einem eigenen Block
  ...
  A_('inserat-speichern', (el) => {
    ...
    const daten = {
      ...
      energie: istGrund ? null : {
        ...
        kwh, art: 'Bedarfsausweis', heizung, ausweisBis: U.isoDate(U.addDays(TT.now(), 3000))
      },
      ausstattung,
      freiAb: (istKauf ? f.get('freiAbKauf') : f.get('freiAb')) || U.isoDate(TT.now()),
      befristetBis: null,
      ...
      anbieter: {
        ...
        verifiziert: true, seit: U.isoDate(TT.now()), inserate: 1, bewertung: 0, bewertungen: 0
      },
      stats: { aufrufe: 0, bewerber: 0, online: U.isoDate(TT.now()) },
      vergleichsmiete: istKauf ? 0 : TT.geo.vergleichsmiete(d.key, flaeche, baujahr, false),
      besichtigungen: [],
      verdacht: false
    };
  ...
  A_('rechner', (el) => {
    ...
    TT.viewWerkzeuge.reglerText(el, einheit === '€' ? U.eur(rWerte[el.dataset.feld])
