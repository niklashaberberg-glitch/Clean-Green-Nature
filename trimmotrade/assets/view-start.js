...
(function (TT) {
  ...
  function aufgaben() {
    ...
    if (!s.profilAngelegt) out.push({
      ...
      text: 'Erst damit sortiert TrimmoTrade nach deiner Passung statt nach Zufall.', ziel: 'profil', knopf: 'Ausfüllen'
    });
    ...
    if (s.einzugsdatum) {
      const plan = S.umzugsPlan().filter((a) => !a.erledigt && a.faellig <= U.isoDate(U.addDays(TT.now(), 14)));
  ...
  function ansicht() {
    ...
    const gesehen = s.gesehen.map((id) => TT.data.byId[id]).filter(Boolean).slice(0, 4);
    const tauschAngebote = TT.data.listings.filter((x) => x.kind === 'tausch');
    ...
    return {
      ...
      html: h`<div class="seite">
        ...
        <section class="block">
          <h2>${ico('ring')}Was TrimmoTrade anders macht</h2>
          <div class="vorteile">
            <article><span class="vorteile__zeichen">${ico('ring')}</span>
              <b>Ringtausch statt Sackgasse</b>
              <p>Der direkte Wohnungstausch scheitert am doppelten Zufall. TrimmoTrade sucht Ketten über mehrere
            ...
            <article><span class="vorteile__zeichen">${ico('blatt')}</span>
              <b>Vertragslupe</b>
              <p>Staffelmiete, Schönheitsreparaturen, Abstandszahlung, Kaution über drei Mieten: TrimmoTrade liest
            ...
            <article><span class="vorteile__zeichen">${ico('kopieren')}</span>
              ...
              <p>Dieselbe Wohnung steht oft zweimal im Angebot, von zwei Maklern. TrimmoTrade merkt das und sagt es,
        ...
        <section class="block">
          ...
          <div class="werkzeuge werkzeuge--klein">
            ${TT.viewWerkzeuge.KATALOG.slice(0, 4).map((k) => h`<a class="werkzeugkachel" href="#/${k.route}">
