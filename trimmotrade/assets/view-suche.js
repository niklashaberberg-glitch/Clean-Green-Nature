...
(function (TT) {
  ...
  function zaehle(f) {
    const s = S.get();
    return A.filtern(TT.data.listings.concat(s.eigeneInserate), f, s.profil).length;
  }
  ...
  function topBlock(bewertet) {
    const top = bewertet.filter((x) => TT.plan.istTop(x.l)).slice(0, TT.plan.TOP_MAX);
  ...
  function ergebnisListe(bewertet) {
    ...
    ${rest > 0 ? h`<div class="mehr">
      ...
      <p class="fein">${U.t('{0} von {1} Treffern geladen')
        .replace('{0}', U.num(teil.length)).replace('{1}', U.num(organisch.length))}</p>
  ...
  function rankingErklaerung(bewertet) {
    ...
    return h`<details class="erklaerung">
      ...
      <p>TrimmoTrade sortiert nach deinem Profil, nicht nach bezahlter Platzierung. Für dieses Inserat zählen vor allem
  ...
  function ansicht(route) {
    ...
    if (route.params.viertel) {
      const d = TT.geo.districtByKey[route.params.viertel];
    ...
    return {
      ...
      danach() {
        ...
        if (!flaeche) return;
        karte = TT.karte.erzeugen(flaeche, {
  ...
  A_('filter-aus-profil', () => {
    ...
    ui.neuZeichnen();
    const treffer = A.filtern(TT.data.listings.concat(s.eigeneInserate), neu, s.profil).length;
  ...
  A_('agent-aus-filter', () => {
    ...
    ui.dialog({
      ...
      inhalt: h`<p>TrimmoTrade merkt sich diese Filter und zeigt dir beim nächsten Besuch, was seither neu hinzugekommen ist.</p>
        <label class="feld"><span>Name</span><input type="text" id="agent-name" value="${vorschlag}" maxlength="80"></label>
        <p class="hinweis">${A.filtern(TT.data.listings, f, S.get().profil).length} Objekte passen aktuell.</p>`,
