/* =====================================================================
   TrimmoTrade – Ansicht: Ringtausch
   ...
   C in die Wohnung von A. TrimmoTrade sucht diese Ketten automatisch.
   ===================================================================== */
(function (TT) {
  'use strict';
  ...
  function meinAngebot() {
    ...
    if (!m) {
      return h`<section class="block block--betont">
        ...
        <p>Ohne eigene Wohnung im Topf kann TrimmoTrade dich in keine Kette einbauen. Das Anlegen dauert zwei Minuten.</p>
        <p><button type="button" class="knopf" data-tu="tausch-anlegen">${ico('plus')}Angebot anlegen</button></p>
      </section>`;
    }
    const alle = TT.data.listings.filter((x) => x.kind === 'tausch').concat([m]);
    const einseitig = M.einseitig(alle, m);
    const schrauben = M.stellschrauben(TT.data.listings.filter((x) => x.kind === 'tausch'), m);
    return h`<section class="block block--betont">
      ...
      ${schrauben.length ? h`<h3>Was mehr Ketten bringen würde</h3>
        ...
        ? h`<ul class="schrauben">
          ...
          <p class="fein">TrimmoTrade hat jede Lockerung einzeln durchgerechnet – das sind keine Schätzungen.</p>`
        : h`<ul class="schrauben schrauben--verdeckt">
            ${schrauben.slice(0, 4).map((s2) => h`<li><span>${s2.label}</span><b>+? Ketten</b></li>`)}
          </ul>
          ${ui.sperrHinweis('stellschrauben', 'TrimmoTrade hat jede dieser Lockerungen einzeln durchgerechnet. '
  ...
  function ansicht() {
    const s = S.get();
    const angebote = TT.data.listings.filter((x) => x.kind === 'tausch').concat(s.meinTausch ? [s.meinTausch] : []);
      ...
      html: h`<div class="seite seite--schmal">
        <header class="seite__kopf">
          ...
          <p class="seite__unter">Beim direkten Tausch müssen zwei Menschen exakt das Gegenteil voneinander wollen –
            ...
            TrimmoTrade durchsucht alle Angebote nach solchen geschlossenen Ketten.</p>
        ...
        <section class="block">
          ...
          <div class="ergebnisse__liste ergebnisse__liste--drei">
            ${TT.data.listings.filter((x) => x.kind === 'tausch').slice(0, 9)
  ...
  A_('ring-anstossen', (el) => {
    const r = TT.viewTausch.letzte[Number(el.dataset.i)];
    if (!r) return;
    ...
    ui.dialog({
      ...
      breit: true,
      inhalt: h`<p>TrimmoTrade hat eine Nachricht vorbereitet, die an alle Beteiligten geht.
  ...
  function erklaerungInhalt() {
    const s = TT.ringBild.SCHRITTE[schritt];
    return h`<div class="ringerklaerung">
      <ol class="ringerklaerung__punkte">
        ${TT.ringBild.SCHRITTE.map((x, i) => h`<li class="${i === schritt ? 'is-an' : i < schritt ? 'is-durch' : ''}">
  ...
  function erklaerungFuss() {
    const letzter = schritt >= TT.ringBild.SCHRITTE.length - 1;
    return h`<button type="button" class="knopf knopf--still" data-tu="ring-schritt" data-n="${schritt - 1}"
      ...
      <span class="fein">Schritt ${schritt + 1} von ${TT.ringBild.SCHRITTE.length}</span>
      ${letzter
  ...
  A_('ring-schritt', (el) => {
    ...
    if (n < 0 || n >= TT.ringBild.SCHRITTE.length) return;
    schritt = n;
    erklaerungZeichnen();
  });
