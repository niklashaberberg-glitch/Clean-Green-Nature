/* =====================================================================
   TrimmoTrade – Ansicht: Dokumententresor und Freigaben
   ===================================================================== */
(function (TT) {
  'use strict';

  const U = TT.util, ui = TT.ui, S = TT.store, T = TT.tresor;
    ...
    return {
      ...
      danach(wurzel) {
        ...
        T.freigabeAbrufen(id, geheim).then((ergebnis) => {
          ...
          TT.viewTresor.geoeffnet = ergebnis.dokumente;
        }).catch((e) => {
  ...
  A_('freigabe-erstellen', (el) => {
    ...
    T.freigabeErstellen({
      ...
      TT.viewTresor.letzterLink = f.link;
      laden();
  ...
  A_('freigabe-ansehen', (el) => {
    const d = (TT.viewTresor.geoeffnet || [])[Number(el.dataset.i)];
    if (!d) return;
    const url = URL.createObjectURL(new Blob([d.bytes], { type: d.typ || 'application/octet-stream' }));
    ui.dialog({
  ...
  TT.viewTresor = { laden, freigabeDialog, freigabeAbschnitt, verweisJeEmpfaenger,
